...
ENABLE_AUTOMATIONS_ROUTER = ENABLE_AUTOMATIONS and not OPEN_WEBUI_LITE_MODE
ENABLE_CALENDAR_ROUTER = ENABLE_CALENDAR and not OPEN_WEBUI_LITE_MODE
ENABLE_TERMINAL_SERVER_ROUTES = ENABLE_TERMINAL_SERVERS and not OPEN_WEBUI_LITE_MODE

if ENABLE_AUTOMATIONS_ROUTER:
    from open_webui.routers import automations
else:
    automations = None

if ENABLE_CALENDAR_ROUTER:
    from open_webui.routers import calendar
else:
    calendar = None

if ENABLE_TERMINAL_SERVER_ROUTES:
    from open_webui.routers import terminals
else:
    terminals = None
...
async def lifespan(app: FastAPI):
    ...
    if ENABLE_AUTOMATIONS_ROUTER or ENABLE_CALENDAR_ROUTER:
        from open_webui.utils.automations import scheduler_worker_loop

        asyncio.create_task(scheduler_worker_loop(app))
    else:
        log.info('Skipping automation/calendar scheduler worker because those routers are disabled.')
